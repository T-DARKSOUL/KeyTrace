    ...
    def _build(self):
        ...
        if icon_path.exists():
            ...
            logo_lbl.setStyleSheet(
                "font-size: 24px; font-weight: bold; color: #00d4aa;"
        ...
        version_lbl.setStyleSheet(
            "font-size: 12px; color: #00d4aa; letter-spacing: 2px;"
            "background: transparent; border: none;"
        )
        tagline_lbl = QLabel("Heat. Shape. Ship.")
        ...
        title_block.addWidget(tagline_lbl)
        ...
        about_layout.addWidget(section_label("About"))
        desc = QLabel(
            "KeyTrace solves the biggest barrier in TLS traffic analysis — the setup. "
            "Configuring SSLKEYLOGFILE correctly stops most people before they even start. "
            "KeyTrace handles it in one click and keeps working with Wireshark and any "
            "other tool outside the app."
        )
        desc.setWordWrap(True)
        desc.setStyleSheet(f"color: {t('text2')}; font-size: 13px; line-height: 1.7;")
        ...
        details_layout.addWidget(section_label("Details"))

        # Metric row
        metrics_row = QHBoxLayout()
        metrics_row.setSpacing(10)
        for label, value in [("Version", "1.0.0"), ("Commands", "14"), ("License", "Proprietary")]:
            metric = QFrame()
            ...
            ml.setSpacing(4)
            lbl = QLabel(label)
            lbl.setStyleSheet(f"color: {t('text3')}; font-size: 11px; border: none; background: transparent;")
            val = QLabel(value)
            val.setStyleSheet(f"color: {t('text')}; font-size: 15px; font-weight: bold; border: none; background: transparent;")
            ml.addWidget(lbl)
            ml.addWidget(val)
            metrics_row.addWidget(metric)
        details_layout.addLayout(metrics_row)

        # Table rows
        rows = [
            ("Platform", f"{_platform.system()} · macOS · Windows"),
            ("Python",   f"{_platform.python_version()}+"),
            ("GitHub",   "github.com/T-DARKSOUL/KeyTrace"),
        ]
        for label, value in rows:
            row = QHBoxLayout()
            lbl = QLabel(label)
            lbl.setStyleSheet(f"color: {t('text2')}; font-size: 13px;")
            val = QLabel(value)
            val.setStyleSheet(f"color: {t('text')}; font-size: 13px; font-weight: bold;")
        ...
        avatar.setStyleSheet(
            f"background-color: {t('accent2')}; color: {t('bg')};"
            "border-radius: 21px; font-size: 14px; font-weight: bold;"
        )
        author_layout.addWidget(avatar)

        author_info = QVBoxLayout()
        author_info.setSpacing(2)
        name = QLabel("Emmanuel Peculiar Ojo")
        name.setStyleSheet(f"color: {t('text')}; font-size: 14px; font-weight: bold;")
        handle = QLabel("T-DARKSOUL  ·  github.com/T-DARKSOUL")
        handle.setStyleSheet(f"color: {t('text2')}; font-size: 12px;")
        author_info.addWidget(name)
        author_info.addWidget(handle)
        ...
        github_btn.clicked.connect(
            lambda: __import__('webbrowser').open("https://github.com/T-DARKSOUL/KeyTrace")
        )

        bug_btn = QPushButton("Report a bug")
        bug_btn.clicked.connect(
            lambda: __import__('webbrowser').open("https://github.com/T-DARKSOUL/KeyTrace/issues")
        )

        contact_btn = QPushButton("Contact")
        contact_btn.clicked.connect(
            lambda: __import__('webbrowser').open("mailto:[ojo39299@gmail.com]")
        )

        for btn in [github_btn, bug_btn, contact_btn]:
